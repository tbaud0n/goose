...
// list all migration scripts in the
// migrations folder, and key them by version
func ListMigrations(dirpath string, applied map[int64]bool) (m []*Migration, err error) {
...
func printQuery(query string) {
	if strings.HasPrefix(query, sqlCmdPrefix) {
